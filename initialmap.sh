...
function dns44380() {
        ###Performs enumeration on standard DNS port 53. Looks for open port with SYN scan. If not open via opencheck function,
        ###looks for R(Reset) flag with ACK scan. If R flag is recieved without an ACK, it can be assumed
        ...
        ###If ACK scan yielded a R(Reset) response from target(showing unfiltered state), BUT THEN shows filtered state as function continues,
        ###it can be assumed the IPS has blocked your IP. Try manual enumeration using different IP.
        echo ; echo "Performing ACK scan on port 53..."
        sudo nmap -sA -Pn -n --disable-arp-ping -p 53 $ip --packet-trace >> autoscan && Rcheck ; [[ $? = 2 ]] && 53Udp
        echo ; echo "Performing SYN scan on port 53..."
        sudo nmap -sS -Pn -n --disable-arp-ping -p 53 $ip --packet-trace --source-port 80 >> autoscan && opencheck ; [[ $? = 2 ]] && return 2
        echo ; echo "Performing ACK scan on port 53..."
        sudo nmap -sA -Pn -n --disable-arp-ping -p 53 $ip --packet-trace --source-port 80 >> autoscan && Rcheck ; [[ $? = 2 ]] && 53Udp
        echo ; echo "Performing SYN scan on port 53..."
        sudo nmap -sS -Pn -n --disable-arp-ping -p 53 $ip --packet-trace --source-port 443 >> autoscan && opencheck ; [[ $? = 2 ]] && return 2
        echo ; echo "Performing ACK scan on port 53..."
        sudo nmap -sA -Pn -n --disable-arp-ping -p 53 $ip --packet-trace --source-port 443 >> autoscan && Rcheck ; [[ $? = 2 ]] && 53Udp
...
function 53Udp() {
        ###Performs datagram scan on port 53/udp, looking for an open state. If open, 
        ###performs intense version scan with script-trace in order to capture as much data as possible; looks for version number.
        ...
        if [[ $? = 2 ]]; then sudo nmap -sUV -p 53 -Pn -n --version-intensity 9 --disable-arp-ping $ip --script-trace --source-port 80 >> sUV\
        && cat sUV | grep -w report -A 100 ; fi && return 2
        ...
        if [[ $? = 2 ]]; then sudo nmap -sUV -p 53 -Pn -n --version-intensity 9 --disable-arp-ping $ip --script-trace --source-port 443 >> sUV\
        && cat sUV | grep -w report -A 100 ; fi && return 2
}
...
function scanprogress() {
        ...
        do percent=$(tail -n 1 allportscan | grep -w About | cut -d':' -f2 | cut -d' ' -f3) ; printf "\r%s" "$percent" ; sleep 1 ; done
...
function scanprogressVersion() {
        ...
        trap "kill $pid 2> /dev/null" EXIT
        while kill -0 $pid 2> /dev/null;
        do percent=$(tail -n 1 nmapScan_sV_openports | grep -w About | cut -d':' -f2 | cut -d' ' -f3) ; printf "\r%s" "$percent" ; sleep 1 ; done
        [[ $? = 0 ]] && echo -en "\033[2k" && printf "\r%s" "100%"
        trap - EXIT
}
...
if [[ -n $openports ]]; 
then sudo nmap -sV --version-intensity 9 -Pn -n $ip -p $openports --reason --stats-every 1s >> nmapScan_sV_openports & scanprogressVersion ; fi
echo ; [[ -n $openports ]] && cat nmapScan_sV_openports | grep -w PORT -A 100 || echo "No open ports found."



port53=$(cat allportscan | grep -w "filtered" | grep -w "53" | grep ^[0-9] | cut -d'/' -f1 | tr '\n' ',' | sed s/,$//) ; [[ -n $port53 ]] && echo && echo\
&& echo -e "${txtgreen}~~~~~~~~~~Beginning enumeration scans on DNS port!~~~~~~~~~~${txtrst}" && dns44380



HTTP=$(cat allportscan | grep -w "filtered" | grep -w "http" | grep ^[0-9] | cut -d'/' -f1 | tr '\n' ',' | sed s/,$//)
[[ -n $HTTP ]] && port=$HTTP && filteredscan && [[ $? = 2 ]] && beginvuln && httpenum_source 
