...
sudo nmap -sC -n -Pn --disable-arp-ping --min-rate 100 --stats-every 1s -p- $ip 2> /dev/null >> allportscan & scanprogress && echo && cat allportscan | grep -w PORT -A 100
...
openports=$(cat allportscan | grep -w "open" | grep ^[0-9] | cut -d'/' -f1 | tr '\n' ',' | sed s/,$//) ; [[ -n $openports ]] && sudo nmap -sV --version-intensity 9 -Pn -n $ip -p $openports --reason --stats-every 1s 2> /dev/null >> nmapScan_sV_openports & scanprogressVersion && echo
...
filteredports=$(cat allportscan | grep -w "filtered" | grep ^[0-9] | grep -v "53" | grep -v "http" | cut -d'/' -f1 | tr '\n' ',' | sed s/,$//)
port=$filteredports ; [[ -n $port ]] && filteredscan
